...
def GeneratKeyPackage(user_id: str):
    ...
    # 2. Credential
    identity_vl = VLBytes(bytes(user_id, "utf-8") + b"@example.com")
    ...
    try:
        ...
        #    f.write(kp_bytes)
        #print("Saved to: alice_keypackage.bin")
        print("First 32 bytes (hex):", kp_bytes[:32].hex())
        response = requests.post(
            f"http://localhost:8000/key_packages/{user_id}",
            data=kp_bytes,
            headers={
                "Content-Type": "application/octet-stream"   # ← this tells FastAPI: "this is raw binary"
            }
        )
        print(response.status_code)
        print(response.text)
        return ed25519_priv_bytes
